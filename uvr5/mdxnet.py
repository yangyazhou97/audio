...
class Predictor:
    ...
    def demix_base(self, mixes, margin_size):
        ...
        for mix in mixes:
            ...
            mix_waves = torch.tensor(
                np.array(mix_waves), dtype=torch.float32).to(self.device)
            with torch.no_grad():
                _ort = self.model
                ...
                if self.args.denoise:
                    ...
                    tar_waves = model.istft(
                        torch.tensor(_ort.run(None, {'input': input_data})[0]).cuda()
                    )
                tar_signal = (
                    tar_waves[:, :, trim:-trim]
                    .transpose(0, 1)
    ...
    def prediction(self, m, vocal_root,format):
        ...
        opt = sources[0].T
        sf.write(
                "%s/vocal_%s" % (vocal_root, basename), mix - opt, rate
            )
...
class MDXNetDereverb:
    ...
    def _path_audio_(self, input, others_root, vocal_root, format,is_hp3=False):
        self.pred.prediction(input, vocal_root,format)